...
{
	double xx = x[0];
	return par[0]*std::exp( -0.5*MUTOO_MACRO::SQUARE( (xx-par[1])/par[2] ) )+ par[3];
}
...
{ 
  return 
    par[0]*std::exp( -0.5*MUTOO_MACRO::SQUARE( (*x-par[1])/par[2] ) )+ 
    par[3]*std::exp( -0.5*MUTOO_MACRO::SQUARE( (*x-par[4])/par[5] ) ); 
}
...
{
	double f = par[0]*TMath::Erf((*x-(par[1]-par[2]))/par[3])-par[0]*TMath::Erf((*x-(par[1]+par[2]))/par[3]);
	f+= par[4]*std::exp( -0.5*MUTOO_MACRO::SQUARE( (*x-par[5])/par[6] ) );
	return f;
}
...
{
  return par[0]/(par[2]*sqrt(2.0*TMath::Pi()))*std::exp( -0.5*MUTOO_MACRO::SQUARE( (*x-par[1])/par[2] ) );
}

//_______________________________________________________________________________
double FitUtils::gaus_int_exp( double *x, double *par) 
{
  double g = par[0]/(par[2]*sqrt(2.0*TMath::Pi()))*std::exp( -0.5*MUTOO_MACRO::SQUARE( (*x-par[1])/par[2] ) );
...
{
  double g1 = par[0]/(par[2]*sqrt(2.0*TMath::Pi()))*std::exp( -0.5*MUTOO_MACRO::SQUARE( (*x-par[1])/par[2] ) );
  double g2 = par[3]/(par[5]*sqrt(2.0*TMath::Pi()))*std::exp( -0.5*MUTOO_MACRO::SQUARE( (*x-par[4])/par[5] ) );
...
double FitUtils::fixed_double_gaus( double *x, double *par )
{
  return 
    par[0]*( 
      (1.0-par[3])/(par[2]*sqrt(2.0*TMath::Pi()))*std::exp(-0.5*MUTOO_MACRO::SQUARE( (x[0] - par[1])/par[2] ) )
    	+ par[3]/(par[4]*sqrt(2.0*TMath::Pi())) * std::exp(-0.5*MUTOO_MACRO::SQUARE( (x[0] - par[1])/par[4] ) )
