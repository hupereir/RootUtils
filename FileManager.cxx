...
bool FileManager::CheckFiles( bool remove_invalid ) const
{
  ...
  void FileManager::Merge( TString output, TString selection, TString option ) const
  {
    ...
    for( const auto& filename:fFiles )
    {
      ...
      if( !( f && f->IsOpen() ) )
      {
        std::cout << "FileManager::Merge - troubles with TFile \"" << filename << "\"." << std::endl;
        continue;
      }
    ...
    for( const auto& treename:treenames )
    {
      ...
      auto chain(GetChain(treename));
      if( chain && chain->GetEntries() )
      {
        
        std::cout << "FileManager::Merge - writing "  <<  treename << std::endl;
        
        /*
        need to recreate the TFile every time because it gets automatically deleted
        in the method (from documentation)
        */
        auto f = TFile::Open( output, (first) ? "RECREATE":"UPDATE" );
        chain->Merge( f, 0, option );
        first = false;
        
      } else std::cout << "FileManager::Merge - chain " << treename << " failed." << std::endl;
    ...
    for( const auto& hname:hnames)
    {
      ...
      auto h( GetHistogram(hname) );
      if( !h ) continue;
      
      std::cout << "FileManager::Merge - writing "  <<  hname << std::endl;
      std::unique_ptr<TFile> out( TFile::Open( output, first ? "RECREATE":"UPDATE" ) );
      first = false;
      out->cd();
      h->Write( h->GetName() );
    }
