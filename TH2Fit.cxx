...
int TH2Fit::exec_minuit_command( const char* command, double* pList, int size )
{	
	int error;
...
bool TH2Fit::fit_inverted( TH2* h, const double yMin, const double yMax	) 
{ 
	...
	// set limits
	if( yMax < yMin ) {
		std::cout << "TH2Fit::fit_inverted - INFO: Fit range is histogram range.\n";
		xMin_ =	h_static_->GetXaxis()->GetBinCenter( 1 );
		xMax_ =	h_static_->GetXaxis()->GetBinCenter( h_static_->GetNbinsX() );
	} else {
		xMin_ = yMin;
...
//_____________________________________________________
void TH2Fit::fcn_inv( int& npar, double *gin, double &res, double *par, int flag )
{
	// Get Histogram and related infos 
	if( !h_static_ ) {
		std::cout << "TH2Fit::fcn_inv - FATAL: histogram not set" << std::endl;
		return;
	}
	unsigned int nX = h_static_->GetNbinsX();
	unsigned int nY = h_static_->GetNbinsY();
	...
	// Get Function and set parameters 
	if( !f_static_ ) {
		std::cout << "TH2Fit::fcn_inv - FATAL: function not set" << std::endl;
		return;
	}
